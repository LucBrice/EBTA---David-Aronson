...
- Between `/start` and `/continue`: run an architecture-refinement loop on the plan via `code-architecture-evaluator` (`/evaluate`) — fix what it flags, re-run `/evaluate`. Minimum 2 passes (a single pass that finds nothing is not proof of convergence); the loop only ends on genuine convergence (a pass that surfaces no new major blind spot), hard-capped at 5-6 passes. If issues are still surfacing at the cap, stop and escalate to the human — the plan itself needs a decision, not another automated pass. Once converged, commit the refined plan as a pre-implementation baseline (topic-scoped commit matching this repo's existing style, e.g. commit `184b013`). Only then does `/continue` begin implementation.
