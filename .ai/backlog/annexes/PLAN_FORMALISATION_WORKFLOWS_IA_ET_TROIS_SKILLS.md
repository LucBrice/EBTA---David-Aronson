...
| **EN ATTENTE** | **D1** — Autoriser le retrait de regles d'`AGENTS.md` au profit de `core-engine/WORKFLOW.md` ? | Leve (ou maintient) le verrou de la section 0. Bloque la Phase 4. |
| **EN ATTENTE** | **D2** — Quel decoupage de la boucle `/evaluate` entre regle universelle et regle `core-engine` (option 1 ou 2 ; option 3 ecartee) ? | Determine le contenu de la table d'inventaire de la Phase 2. Meme question pour la regle `epic-orchestrator`. |
| **EN ATTENTE** | **D3** — Router `_call_float` comme chantier `fix` separe ? | **Oui** : `adversarial-tester/EXAMPLE_REPORT.md` est construit sur `_call_float` et pointe vers ce chantier. **Non** : l'`EXAMPLE_REPORT.md` bascule sur le repli defini en Phase 1 (defaut historique **deja corrige**), et `_call_float` n'est mentionne nulle part — un defaut vivant ne peut pas servir de materiau documentaire sans porteur. |
| **EN ATTENTE** | **D4** — Le Fil ouvert D est-il seulement rendu visible via `CONFLICT_RESOLUTION_POLICY.md`, ou arbitre maintenant ? | Determine le contenu de la note de gouvernance dans `interface/WORKFLOW.md`. |
