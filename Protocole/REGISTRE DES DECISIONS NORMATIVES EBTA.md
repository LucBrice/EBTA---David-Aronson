...
| DN-005 | Identité d’une candidate | Une candidate est une spécification exécutable unique ayant produit une information de performance exploitable ou sélectionnable ; lorsque l’actif est sélectionnable, le couple `stratégie × actif` fait partie de cette identité. | SOP 03 | SOP 02, 05, 06, 09A, 09B, 12 | Rend opposable l’effort réel de recherche. | Fixe | Avant première évaluation exploitable. | Registre append-only, représentation canonique, `candidate_id`, `asset_universe`. |
...
| DN-007 | Famille de recherche | La famille inclut toutes les candidates dont les résultats ont pu influencer une même décision sur des données de sélection partagées, y compris les actifs ou couples `stratégie × actif` séparément sélectionnables. | SOP 03 | SOP 02, 05, 06, 09A, 12 | Définit la population opposable pour l’inférence multiple. | Fixe | Avant WRC local. | `RESEARCH_FAMILY_ID`, catalogue, `asset_universe`, matrice scellée. |
| DN-008 | Population testée par WRC | Le WRC local porte sur l’univers complet et reconstructible des candidates applicables à `Test_k`, sans réduire ex post les actifs ou couples évalués. | SOP 02 | SOP 03, 06, 12 | Corrige le biais de data mining lié à la sélection de la meilleure règle ou du meilleur couple `stratégie × actif`. | Fixe | Avant exécution du WRC local. | Matrice `Test_k`, hash du catalogue, rapport WRC. |
...
| --- | --- | --- |
| Univers, actifs, `asset_universe`, période et calendrier | SOP 03, SOP 04, SOP 09A | Avant création des splits et avant première évaluation exploitable. |
