...
| Promouvoir un brouillon avec `/start` | CONDITIONNELLE | Demande humaine, double boucle `/evaluate`, plan réécrit, workflow `ACTIVE`, 2-6 passes et référence intake. | Contrat `WORKFLOW.json`, sections du gabarit et état `TRIAGED`. | `.ai/workflows/common/WORKFLOW.json`; `.ai/workflows/common/WORKFLOW.md` |
| Attester une baseline avec `plan.ps1 baseline` | CONDITIONNELLE | Plan normalisé convergé, commit Git existant contenant le `source_path`, 2-6 passes et référence d'audit. | Transition `TRIAGED` vers `BASELINED`, puis commit séparé de l'état. | `.ai/workflows/common/WORKFLOW.json`; `.ai/workflows/common/WORKFLOW.md` |
| Reprendre un plan avec `/continue` | CONDITIONNELLE | Stage `BASELINED` et test `epic-orchestrator` rejoué. | Transition machine vers `ACTIVE`, carte d'exécution et workflow spécialisé. | `.ai/workflows/common/WORKFLOW.json`; `.ai/workflows/common/WORKFLOW.md` |
| Déclarer un plan prêt avec `plan.ps1 ready` | CONDITIONNELLE | Tous les gates applicables ont été réellement exécutés et chaque ID requis référence une preuve honnête. | Transition `ACTIVE` vers `READY_TO_CLOSE`; présence mécanique ne vaut pas validation sémantique. | Contrat `WORKFLOW.json` applicable; `WORKFLOW.md` associé |
| Fermer un plan avec `/close` | CONDITIONNELLE | `DONE` exige `READY_TO_CLOSE`; un outcome terminal non nominal doit être explicitement justifié. | JSON d'état validés puis commit limité à la clôture ; jamais de push automatique. | `.ai/workflows/common/WORKFLOW.json`; `.ai/workflows/common/WORKFLOW.md`; workflow spécialisé applicable |
...
| Créer la baseline pré-implémentation | CONDITIONNELLE | Plan normalisé convergé après au moins deux passes post-`/start`; fichiers machine valides. | Forme détaillée du commit, scope intentionnel, puis action `baseline`. | `.ai/workflows/common/WORKFLOW.md` |
