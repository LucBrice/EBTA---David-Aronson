...
| --- | --- | --- | --- | --- | --- |
| `search_space` | SOP 03, SOP 06, SOP 09A, DN-005 to DN-007 | Pre-registered candidate space, fold id, train segment id, parameter grid, optional `asset_universe` and `asset_selection_axis` | Canonical candidate specifications and search-space snapshot, including `candidate_asset_map` when assets are selectable | `reports/search_space.json` | No ex post candidate merge; no OOS input; no asset selected outside the preregistered universe |
...
| `candidate_matrix` | SOP 02, SOP 03, SOP 06, DN-007, DN-008 | Full applicable candidate list, Test_k series by candidate, optional candidate-to-asset map | Complete matrix for WRC family, including complete `strategie x actif` coverage when active | `reports/candidate_matrix.json` | No winner-only matrix; no omitted influential candidate; no omitted evaluated asset |
